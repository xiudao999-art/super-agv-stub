    ...
    Action<string>? logger = null)
{
        ...
        ActionTemplateContext context, CancellationToken ct)
    {
        ...
        foreach (var phase in template.Phases)
        {
            if (!phase.Enabled)
            {
                steps.Add(CreateStep(steps.Count + 1, phase, "DISABLED"));
                continue;
            }
            ...
            if (completedPhaseIds.Contains(phase.PhaseId))
            {
                ...
                steps.Add(CreateStep(steps.Count + 1, phase, "RESUMED_SUCCEEDED"));
                continue;
            }

            var maxRetries = ResolveMaxRetries(phase, context.Policy);
            PhaseResult result = new(false, null, new DeviceError(5002, $"phase {phase.PhaseId} 尚未执行。"));
            for (var attempt = 0; attempt <= maxRetries; attempt++)
            {
                ...
                steps.Add(CreateStep(steps.Count + 1, phase,
                    ...
                    string.Equals(context.Policy?.RetryMode, "VERIFY_BEFORE_RETRY", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    if (verification.Decision == RetryVerificationDecision.Abort)
                    {
                        result = new(false, verification.Evidence, verification.Error ??
                            new DeviceError(5002, $"phase {phase.PhaseId} 复核结果不允许自动重试。",
                    ...
                    if (attempt >= maxRetries)
                    {
                        result = new(false, verification.Evidence,
                            CreateExhaustionError(phase, context.Policy));
                        steps.Add(CreateStep(steps.Count + 1, phase,
                ...
                {
                    result = new(false, result.Evidence, CreateExhaustionError(phase, context.Policy));
                    steps.Add(CreateStep(steps.Count + 1, phase,
            ...
            if (!result.Success && stepEvidence is PhaseFailureEvidence)
                steps.Add(CreateStep(steps.Count + 1, phase, "CAPTURED", stepEvidence, "FAILURE_EVIDENCE"));
            ...
            return new(false, steps, result.Error ?? new DeviceError(5002, $"phase {phase.PhaseId} 失败。"));
        }
        ...
        PhaseActionTemplate failedPhase, ActionTemplateContext context, CancellationToken ct)
    {
            ...
            if (template.ActionType is MainAction.ArmPick or MainAction.ArmPickBatch)
            {
                ...
                return source.Success
                    ...
                    : new(RetryVerificationDecision.Abort, source.Evidence,
                        new DeviceError(5201, "夹爪未检测到物料，且视觉无法确认物料仍在原位；物理结果未知，禁止盲重试。",
                            PhysicalResultKnown: false, Retryable: false));
            }
            ...
            if (template.ActionType is MainAction.ArmPlace or MainAction.ArmPlaceBatch)
            {
                ...
                return placed.Success
                    ? new(RetryVerificationDecision.Abort, placed.Evidence,
                        new DeviceError(5202, "视觉确认已放置，但夹爪仍检测到负载；传感器结果冲突，需人工复核。",
                            PhysicalResultKnown: false, Retryable: false))
        ...
        if (template.ActionType is MainAction.ArmPick or MainAction.ArmPickBatch)
        {
            ...
            var result = Convert(await vision.VerifyAsync(request, ct), x => x?.Passed == true);
        ...
        {
            var result = Convert(await chassis.GetStatusAsync(ct), x => x is { Connected: true, Moving: false });
            return result.Success ? new(RetryVerificationDecision.Satisfied, result.Evidence)
                : new(RetryVerificationDecision.RetryFrom, result.Evidence, result.Error);
        }

        var armResult = Convert(await arm.GetStatusAsync(ct), x => x is { Connected: true, Moving: false });
        ...
        string failedPhaseId, ActionTemplateContext context, List<OperationStep> steps, CancellationToken ct)
    {
        ...
        if (from < 0 || failed < 0 || from >= failed)
            return new(false, null, new DeviceError(4000,
        ...
        for (var index = from; index < failed; index++)
        {
            ...
            steps.Add(CreateStep(steps.Count + 1, replayPhase,
    ...
    private static DeviceError CreateExhaustionError(PhaseActionTemplate phase, ActionExecutionPolicy? policy)
    {
        var onExhaust = ResolveOnExhaust(phase, policy);
        return onExhaust switch
        {
            "CANCEL" => new DeviceError(5211, $"phase {phase.PhaseId} 重试耗尽，动作已取消。",
            ...
            "MANUAL" => new DeviceError(5212, $"phase {phase.PhaseId} 重试耗尽，转人工恢复。",
                PhysicalResultKnown: false, Retryable: false, Category: DeviceErrorCategory.State,
                RecoveryStrategy: DeviceRecoveryStrategy.ManualRecovery,
                HandlingAdvice: "MANUAL：人工确认物料和设备状态后再恢复。"),
            _ => new DeviceError(5210, $"phase {phase.PhaseId} 重试耗尽，动作进入 HOLD。",
                PhysicalResultKnown: false, Retryable: false, Category: DeviceErrorCategory.State,
                RecoveryStrategy: DeviceRecoveryStrategy.ManualRecovery,
        ...
        CancellationToken ct) => phase.SubAction switch
    {
        SubAction.MOVE_TO_MAP_POINT => context.MoveRequest is null
            ? new(false, null, new DeviceError(4000, $"phase {phase.PhaseId} 缺少 MOVE request。"))
            : Convert(await chassis.MoveAsync(context.MoveRequest, ct)),
        ...
        SubAction.VISION_VERIFY_MATERIAL or SubAction.VISION_VERIFY_PLACEMENT =>
            Convert(await vision.VerifyAsync(
                L1SubActionVisionVerifyMaterial.ResolveRequest(phase, context.Station), ct), x => x?.Passed == true),
        ...
        SubAction.CHASSIS_VERIFY_STOPPED => Convert(await chassis.GetStatusAsync(ct), x => x is { Connected: true, Moving: false }),
        SubAction.ARM_VERIFY_HOME => Convert(await arm.GetStatusAsync(ct), x => x is { Connected: true, Moving: false, Homed: true }),
        _ => new(false, null, new DeviceError(4004,
    ...
    private static PhaseResult Convert<T>(DeviceResult<T> result, Func<T?, bool>? predicate = null)
    {
        var success = result.Success && (predicate?.Invoke(result.Value) ?? true);
        return success ? new(true, result.Value) : new(false, result.Value,
            result.Error ?? new DeviceError(5002, "子动作成功条件未满足。"));
    }
